...
for (let btn of deleteBtnList) { // 향상된 for문
    btn.addEventListener("click", e => {
        ...
        // 요소.closest("CSS선택자")
        // - 지정된 요소의 상위 요소(부모 방향, 루트 최상위까지)를
        // 중에서 CSS 선택자가 일치하는 요소를 찾을 때까지 검색
        // 일치하는 요소가 있으면 해당 요소 반환

        const deptId = e.target.closest("tr").children[1].innerText; // 조금 더 짧게 쓰기

        if (confirm(`${deptId}를 정말 삭제 할꺼임?`)) {
            // 확인 클릭 시 

            // 삭제 요청 보내기 1) location.href 이용
            // -> GET   방식 요청
            // -> 브라우저 주소창에 입력하는 방법도 GET 방식 요청
            // --> 임의의 사용자가 삭제 요청을 마음대로 보내는 문제가 발생할 수 있다!
